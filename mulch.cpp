...
std::string to_string(Material material)
{
    if (material == Material::RUBBER)
        return "rubber";
    else if (material == Material::PINE)
        return "pine";
    else if (material == Material::CEDAR)
        return "cedar";
    else if (material == Material::HARDWOOD)
        return "hardwood";
    else
...
Material material_from_string(std::string s)
{
    if (s == "rubber")
        ...
        throw std::out_of_range("Invalid Material from string");
}
...
{ // check lecture 17 canvas.h and canvas.cpp
    ost << "mulch\n";
    Product::save(ost);
    ost << _volume << '\n'
        << ::to_string(_material) << '\n';
